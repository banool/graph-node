...
use graph::blockchain::{
    BasicBlockchainBuilder, Block, BlockIngestor, BlockchainBuilder, BlockchainKind, NoopRuntimeAdapter
};
...
use crate::txnstream::AptosTxnStream;
use crate::{
    codec,
...
impl Blockchain for Chain {
    const KIND: BlockchainKind = BlockchainKind::Aptos;

    type Client = ();

    // Since blocks aren't really relevant for Aptos but it is so ingrained in this codebase
    // I'll just put a single txn in each block.
    type Block = codec::MyBlock;
    ...
    async fn new_block_stream(
        ...
        let stream: AptosTxnStream<Chain> = AptosTxnStream::new(
            deployment.hash,
            Url::from_str("http://127.0.0.1:50051").unwrap(),
            "notused".to_string(),
            Some(0),
            None,
        );
...
impl TriggersAdapterTrait<Chain> for TriggersAdapter {
    ...
    async fn ancestor_block(
        ...
        _ptr: BlockPtr,
        _offset: BlockNumber,
    ) -> Result<Option<codec::MyBlock>, Error> {
...
impl BlockStreamMapper<Chain> for FirehoseMapper {
    fn decode_block(&self, output: Option<&[u8]>) -> Result<Option<codec::MyBlock>, Error> {
        /*
        let block = match output {
            Some(block) => codec::MyBlock::decode(block)?,
...
mod test {
    ...
    fn new_success_block(height: u64, receiver_id: &String) -> codec::MyBlock {
        codec::MyBlock {
